...
local followByPercent = false -- ✅ ตัวแปรใหม่สำหรับเช็คโหมด %
...
-- ✅ [เพิ่มจุดที่ 1] เพิ่มปุ่มเปิด/ปิดการตามด้วย % เลือด
Section:NewToggle("Follow by % HP", "เปิด=คำนวณจาก %, ปิด=คำนวณจากค่าเลือดปกติ", function(state)
    followByPercent = state
end)
...
local function findSafeDodge(startPos, baseDir, distance)
    ...
    for _, angle in ipairs(angles) do
        ...
        if isSafePosition(startPos, testTarget) then return testTarget end
    end
...
local function executeSmartDodgeV5(hazard)
    ...
    elseif isProjectile then
        ...
        if bodyVel then projVelocity = bodyVel.Velocity
        elseif linearVel then projVelocity = linearVel.VectorVelocity
        else projVelocity = hazard.Velocity end
        local speed = projVelocity.Magnitude
        local projDir = Vector3.new(0, 0, 0)
        if speed > 1 then projDir = projVelocity.Unit
        else projDir = hazard.CFrame.LookVector; speed = 60 end
        local flatProjDir = Vector3.new(projDir.X, 0, projDir.Z)
        flatProjDir = flatProjDir.Magnitude > 0 and flatProjDir.Unit or Vector3.new(1, 0, 0)
        ...
        if approachDot > 0.4 then
            ...
            if timeToImpact < 0.6 or distXZ < 12 then
                ...
                if isSafePosition(myPos, myPos + (dodgeRight * dodgeDist)) then safeTarget = myPos + (dodgeRight * dodgeDist)
                elseif isSafePosition(myPos, myPos + (dodgeLeft * dodgeDist)) then safeTarget = myPos + (dodgeLeft * dodgeDist)
                else
                    local forwardDir = -flatProjDir 
                    local warpBehindTarget = myPos + (forwardDir * 15)
                    if isSafePosition(myPos, warpBehindTarget) then safeTarget = warpBehindTarget
                    else safeTarget = findSafeDodge(myPos, forwardDir, 8) end
                end
                if safeTarget then myRoot.CFrame = CFrame.new(safeTarget) end
            end
        end
                                ...
                                elseif item:IsA("Model") then
                                    ...
                                    for _, part in pairs(item:GetDescendants()) do
                                        if part:IsA("BasePart") then
                                            ...
                                            if part:FindFirstChild("TouchInterest") then part.CFrame = myRoot.CFrame end
                                        end
                                    end
                                end
                            end
...
RunService.Stepped:Connect(function()
    if autoDodgeEnabled then
        pcall(function()
            ...
            if effects then for _, v in pairs(effects:GetChildren()) do executeSmartDodgeV5(v) end end
        end)
    end
end)
...
task.spawn(function()
    while true do
        ...
        if autoDodgeEnabled then
            pcall(function()
                if getnilinstances then for _, v in pairs(getnilinstances()) do executeSmartDodgeV5(v) end end
            end)
        end
    end
end)
    ...
    while true do
        ...
        pcall(function()
            ...
            elseif SelectedMode == "Random" then
                ...
                -- ✅ [แก้ไขจุดที่ 2] ปรับปรุงลอจิก Max/Min HP ให้รองรับโหมด % เลือด
                ...
                for _, p in pairs(Players:GetPlayers()) do
                    if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("Humanoid") and p.Character.Humanoid.Health > 0 then
                        ...
                        if SelectedMode == "Max HP" or SelectedMode == "Min HP" then
                            -- ✅ คำนวณค่าที่จะเอามาเทียบ (เลือกได้ว่าจะเป็น HP ปกติ หรือ %)
                            local currentVal = followByPercent and (hum.Health / hum.MaxHealth) or hum.Health
                            
                            if (SelectedMode == "Max HP" and currentVal > bestHP) or (SelectedMode == "Min HP" and currentVal < bestHP) then
                                bestHP = currentVal; target = p
                            end
